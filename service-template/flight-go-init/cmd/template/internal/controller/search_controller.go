...
type (
	SearchControllerImpl struct {
		deps          deps.Deps
		searchService service.SearchServiceItf
	}
)

func NewSearchController(deps deps.Deps, searchService service.SearchServiceItf) *SearchControllerImpl {
	return &SearchControllerImpl{deps: deps, searchService: searchService}
}
...
func (ctrl *SearchControllerImpl) Search(c echo.Context) error {
	...
	response, err := ctrl.searchService.SearchWithMetric(ctx, mr, request, request.TripTypes[0], request.SupplierMappings[0])
	if err != nil {
	...
	return c.JSON(http.StatusOK, common.ConstructResponse[fareRS.FlightIntegratorSearchResponse](shared.SUCCESS_RESPONSE_CODE, shared.SUCCESS_RESPONSE_CODE, nil, response))
}
...
// @Success		200	{object}	fareRS.FlightIntegratorSearchResponse
// @Failure		400
// @Router			/{{BASE_PATH}}/v2/search [post]
func (ctrl *SearchControllerImpl) SearchV2(c echo.Context) error {
	...
	response, err := ctrl.searchService.SearchAndPublish(ctx, mr, request, request.TripTypes[0], request.SupplierMappings[0])
	if err != nil {
		ctrl.deps.GetLogger(ctx).Error(util.LogRest(mr, ctxOperation, shared.RESPONSE, err.Error()))
		// our existing java code return 200 with error message
		return c.JSON(http.StatusOK, common.ConstructResponse[*fareRS.FlightIntegratorSearchResponse](GetSearchResponseCodeMapper(err), err.Error(), []string{err.Error()}, nil))
