...
type (
	...
	{{CONFIG_PROPERTY_STRUCT}} struct {
		...
		OutBoundConfig OutBoundConfig `yaml:"outbound_config"`
	}
	OutBoundConfig struct {
		DummyConfig config.ClientHttpConfig `yaml:"dummy_config"` // todo - place holder replace with actual when implementing outbounds
	}
)
