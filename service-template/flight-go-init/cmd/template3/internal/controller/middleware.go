...
func HawkEyeAPIInbound(deps deps.Deps) echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			if err != nil {
				go func(t time.Time) {
					err := deps.Metric.CustomMonitorLatency(c.Request().URL.RequestURI(), common.API_IN,
						common.Failed, http.StatusInternalServerError, tags, time.Since(t))
					if err != nil {
			...
			go func(t time.Time) {
				err := deps.Metric.CustomMonitorLatency(c.Request().URL.RequestURI(), common.API_IN,
					common.Success, http.StatusOK, tags, time.Since(t))
				if err != nil {
					deps.GetLogger(nil).Errorf("%s - Error publish to statsd (hawkeye). Got Err: %s", "[StatsD]",
						err.Error())
				}
