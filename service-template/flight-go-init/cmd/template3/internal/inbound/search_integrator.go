...
type integratorListenerImpl struct {
	deps            deps.Deps
	searchProcessor service.SearchProcessorService
...
func (impl *integratorListenerImpl) ListenToIntegratorRequest(data []byte, headers []kafka.Header, _, _ int64) error {
	...
	impl.deps.WorkerPoolWrapper.SearchRequestPool.Go(func() {
		...
		if err := impl.searchProcessor.SearchKafkaProcessor(ctxWithTimeout, message); err != nil {
			errorCode = common.Failed
...
// NewIntegratorListener builds the search integrator listener.
func NewIntegratorListener(deps deps.Deps, searchProcessor service.SearchProcessorService, systemParam system_param.Service) (IntegratorListener, error) {
	return &integratorListenerImpl{deps: deps, searchProcessor: searchProcessor, systemParam: systemParam}, nil
}
