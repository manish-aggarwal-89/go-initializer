...
type (
	IssuedController struct {
		deps deps.Deps
		svc  service.IssuedServiceItf
		bau  bau.AnalyticsService[issuedRQ.IntegratorIssuedRequest]
	}
)

func NewIssuedController(deps deps.Deps, svc service.IssuedServiceItf, bau bau.AnalyticsService[issuedRQ.IntegratorIssuedRequest]) *IssuedController {
...
func (controller *IssuedController) Issued(c echo.Context) error {
	...
	defer func() {
		analyticToken.Complete()
		controller.bau.Complete(mr, analyticToken, err)
	}()
	...
	response, err := controller.svc.Issued(ctx, mr, request)
	if err != nil {
